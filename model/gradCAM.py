...
class ModelOutputs():
    ...
    def __init__(self, model, target_layers):
        ...
        self.feature_extractor = FeatureExtractor(
            self.model.backbone, target_layers)
    ...
    def __call__(self, x):
        ...
        output = self.model.adapool(output).squeeze()
        output = self.model.fc2(output)
...
class GradCam:
    def __init__(self, model, target_layer_names, args):
        self.model = model
        self.model.eval()
        self.args = args
        if self.args.cuda:
    ...
    def __call__(self, input, index=None):
        ...
        cam = cv2.resize(cam, (self.args.input_size, self.args.input_size))
        cam = cam - np.min(cam)
        cam = cam / np.max(cam)
        return cam, output
