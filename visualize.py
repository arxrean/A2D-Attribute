...
def joint_plot(args, thd=0.669):
    ...
    if os.path.exists(dir_path) and int(input('dir exist! delete?(1/0)')) == 1:
    ...
    for iter, pack in enumerate(val_loader):
        ...
        for index in gt:
            ...
            show_cam_on_image(args, os.path.join(args.a2d_root, part_path), mask, os.path.join(dir_path, '1', part_path.split(
                '/')[-1][:-4]+'_'+str(index)+'.jpg') if output[index] == 1 else os.path.join(dir_path, '1', part_path.split('/')[-1][:-4]+'_'+str(index)+'.jpg'))
